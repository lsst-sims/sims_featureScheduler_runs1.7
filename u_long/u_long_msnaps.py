...
def generate_blobs(nside, nexp=2, nexp_dict=None, exptime=30., u_exptime=60., filter1s=['u', 'u', 'g', 'r', 'i', 'z', 'y'],
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        surveys.append(Blob_survey(basis_functions, weights, filtername1=filtername, filtername2=filtername2,
                                   ...
                                   nexp=nexp_dict[filtername], detailers=detailer_list, nexp_dict=nexp_dict,
                                   **blob_survey_params))
...
def u_double_footprint(nside=None):
    ...
    result['u'] = generate_goal_map(nside=nside, NES_fraction=0.,
                                    WFD_fraction=0.67, SCP_fraction=0.30,
                                    GP_fraction=0.30,
                                    wfd_dec_min=-62.5, wfd_dec_max=3.6)
...
if __name__ == "__main__":
    ...
    nexp = 2
    nexp_dict = {'u': 1, 'g': 2, 'r': 2, 'i': 2, 'z': 2, 'y': 2}
    ...
    blobs = generate_blobs(nside, nexp=nexp, nexp_dict=nexp_dict, footprints=footprints, u_exptime=u_expt)
