...
def slice_wfd_indx(target_map, nslice=2):
    """
    simple map split
    """
...
def make_rolling_footprints(mjd_start=59853.5, sun_RA_start=3.27717639, nslice=2, scale=0.8, nside=32):
    hp_footprints = footprint_maker(6)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nslice", type=int, default=6)
    ...
    parser.add_argument("--nrw", type=float, default=0.)
    ...
    fileroot = 'six_stripe_scale%.2f_nslice%i_fpw%.1f_nrw%.1f' % (scale, nslice, fpw, np.abs(nrw))
    if not grow_blob:
        fileroot += '_unc_'
    if nexp != 2:
