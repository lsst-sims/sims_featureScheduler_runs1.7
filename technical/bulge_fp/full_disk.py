...
def gen_greedy_surveys(nside=32, footprints=None,
    ...
    for filtername in filters:
        ...
        bfs.append((bf.Limit_repeat_basis_function(filtername=None, n_limit=2), norepeat_weight))
...
def generate_blobs(nside, nexp=2, footprints=None,
                   ...
                   shadow_minutes=60., max_alt=76., moon_distance=30., ignore_obs='DD',
                   m5_weight=6., footprint_weight=0.6, slewtime_weight=3.,
                   stayfilter_weight=3., template_weight=12., norepeat_weight=-3.,
                   grow_blob=True):
    """
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        surveys.append(Blob_survey(basis_functions, weights, filtername1=filtername, filtername2=filtername2,
                                   ...
                                   nexp=nexp, detailers=detailer_list, grow_blob=grow_blob,
                                   **blob_survey_params))
...
def make_rolling_footprints(mjd_start=59853.5, sun_RA_start=3.27717639, nslice=2, scale=0.8, nside=32):
    hp_footprints = full_disk_fp(nside=nside)
...
def full_disk_fp(nside=32):
    wfd_north_dec = 4
    wfd_south_dec = -62.25
    ra, dec = np.degrees(ra_dec_hp_map(nside=nside))
    wfd_weights = {'u': 0.31, 'g': 0.44, 'r': 1., 'i': 1., 'z': 0.9, 'y': 0.9}
    result = combo_dust_fp(wfd_weights=wfd_weights,
                           nside=nside, outer_bridge_l=250, outer_bridge_width=0., outer_bridge_alt=20.,
                           bulge_lon_span=0., bulge_alt_span=0.,
                           north_weights={'g': 0.0, 'r': 0.0, 'i': 0.0},
                           wfd_north_dec=wfd_north_dec, wfd_south_dec=wfd_south_dec,
                           mc_wfd=False)
    in_range = np.where((dec >= wfd_south_dec) & (dec <= wfd_north_dec))[0]
    for key in result:
        result[key][in_range] = wfd_weights[key]
    return result
...
if __name__ == "__main__":
    ...
    fileroot = 'full_disk_scale%.2f_nslice%i_fpw%.1f_nrw%.1f' % (scale, nslice, fpw, np.abs(nrw))
    if not grow_blob:
        fileroot += '_unc_'
    if nexp != 2:
    ...
    greedy = gen_greedy_surveys(nside, nexp=nexp, footprints=footprints, footprint_weight=fpw,
                                norepeat_weight=nrw)
    blobs = generate_blobs(nside, nexp=nexp, footprints=footprints, footprint_weight=fpw,
                           norepeat_weight=nrw, grow_blob=grow_blob)
